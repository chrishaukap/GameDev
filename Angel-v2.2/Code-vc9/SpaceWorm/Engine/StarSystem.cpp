...
{   
   ...
   const int maxNumParticles = 5;
   std::vector<Particle*> particles;
   for(int i=0; i<maxNumParticles; ++i)
      particles.push_back(new CollidingParticle);

   pa->SetParticles(particles);
   pa->SetParticlesPerSecond(5); //Emission Rate
...
void Star::Update(float dt)
{
   ...
   case Exploding: 
      if(m_timeExistant >= StarExplodeTime)
      {
         ...
         m_particleCluster->SetParticlesPerSecond(0.0f); //Emission Rate
      }
      break;
   default: assert(!"Shouldn't get here");
   }
}
static const float StarSpawnRate = 1.8f;
...
void StarSystem::GetStars(std::vector<const ParticleActor*>& stars) const
{
