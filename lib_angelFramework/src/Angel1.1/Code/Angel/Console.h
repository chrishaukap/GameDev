...
public:
	...
	public:
		...
		virtual bool BoundToObjectHash( CCmdMemberFunctorOwner* funcOwner ) {funcOwner; return false;}
...
class ConsoleItemManager
{
	typedef stdext::hash_map<String, ConsoleVariable*> CVarTable;
	typedef stdext::hash_map<String, ConsoleCommand*> CCmdTable;
public:
	~ConsoleItemManager();
...
#define theConsole DeveloperConsole::GetInstance()

#define CONSOLE_GETVAR( cvar ) \
	theConsole.GetConsoleItemManager().GetCVar( cvar );
...
#define CONSOLE_DECLARECCMDINTERNAL( ccmd, functor, flags ) \
	theConsole.GetConsoleItemManager().AddCCmd( #ccmd, functor, flags );
...
class DeveloperConsole : public CCmdMemberFunctorOwner
{
public:
	...
	bool GetInput(unsigned char key);
	...
	void ExecuteInConsole( const String& input );
	void ExecuteInConsole( const String& input, bool bNoLog );
