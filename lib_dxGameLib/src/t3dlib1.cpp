             ...
             LPDIRECTDRAWSURFACE7 dest) // surface to draw the bob on
{
   ...
   if (!(bob->attr & BOB_ATTR_VISIBLE)) return(1);
...
int Draw_Scaled_BOB(BOB_PTR bob, 
                    int swidth, 
                    int sheight,                // bob and new dimensions
                    LPDIRECTDRAWSURFACE7 dest)  // surface to draw the bob on)
{
...
int Draw_Scaled_BOB16(BOB_PTR bob, 
                      int swidth, 
                      int sheight,  // bob and new dimensions
                      LPDIRECTDRAWSURFACE7 dest) // surface to draw the bob on)
{
...
int Move_BOB(BOB_PTR bob)
{
   ...
   if (bob->attr & BOB_ATTR_WRAPAROUND)
   {
      ...
      else if (bob->x < min_clip_x-bob->width)
         bob->x = max_clip_x;
   ...
   // test for bounce
   else if (bob->attr & BOB_ATTR_BOUNCE)
   {
      // test x extents first
...
int Draw_Line(int x0, 
              int y0,         // starting position 
              int x1, 
              int y1,         // ending position
...
int Draw_Text_GDI(char *text, int x,int y,COLORREF color, LPDIRECTDRAWSURFACE7 lpdds)
{
   ...
   SelectObject( xdc, GetStockObject( SYSTEM_FIXED_FONT));
