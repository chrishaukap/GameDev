...
void TextActor::Render()
{
	...
	for(int i=0; i < _displayStrings.size(); i++)
	{
		DrawGameText(_displayStrings[i]._string, _fontNickname, (int)_displayStrings[i]._position.X, (int)_displayStrings[i]._position.Y);
	}
}

void TextActor::Update(float dt)
{
	//This is horribly inefficient, but it'll have to do until there's a message system we could subscribe to
	CalculatePosition();
}
