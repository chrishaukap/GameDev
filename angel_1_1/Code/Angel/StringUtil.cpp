...
bool StringToBool( const String& s )
{
	...
	if( !_stricmp( s.c_str(), "true") )
		return true;
	else if( !_stricmp(s.c_str(), "false") )
		return false;
...
String TrimString( const String& trimMe, const String& trimChars )
{
	...
	//trim from the front
	int trimStart = -1;
	for( int i = 0; i < trimMe.length(); i++ )
	{
		char ch = trimMe[i];
		if( trimChars.find(ch) == -1)
		{
			trimStart = i;
			break;
		}
	...
	if( trimStart >= 0 && trimStart < (trimMe.length()) )
	{
		for( int i = trimMe.length()-1; i >= 0; i-- )
		{
			char ch = trimMe[i];
			if( trimChars.find(ch) == -1 )
			{
...
void GetCarCdr( StringList& inputStrings, String& car, String& cdr, int numInputStrings )
{
	...
	if( numInputStrings < 0 )
		numInputStrings = inputStrings.size();
