...
{		
	...
	unsigned int numHorizontalUs =   NumCellsInDirection(grid,j,i, Left, us) + NumCellsInDirection(grid,j,i, Right, us);
	unsigned int numVerticalUs =     NumCellsInDirection(grid,j,i, Up, us) + NumCellsInDirection(grid,j,i, Down, us);
	unsigned int diagonal1Us =       NumCellsInDirection(grid,j,i, UpperRight, us) + NumCellsInDirection(grid,j,i, LowerLeft, us);
	unsigned int diagonal2Us =       NumCellsInDirection(grid,j,i, UpperLeft, us) + NumCellsInDirection(grid,j,i, LowerRight, us);
	unsigned int numHorizontalThem = NumCellsInDirection(grid,j,i, Left, them) + NumCellsInDirection(grid,j,i, Right, them);
	unsigned int numVerticalThem =   NumCellsInDirection(grid,j,i, Up, them) + NumCellsInDirection(grid,j,i, Down, them);
	unsigned int diagonal1Them =     NumCellsInDirection(grid,j,i, UpperRight, them) + NumCellsInDirection(grid,j,i, LowerLeft, them);
	unsigned int diagonal2Them =     NumCellsInDirection(grid,j,i, UpperLeft, them) + NumCellsInDirection(grid,j,i, LowerRight, them);
...
bool 
TTTAIUtils::hasNonEmptyNeighbor(const TTTGrid& grid, unsigned int j, unsigned int i)
{
...
TTTAIUtils::getPossibleMoves(const TTTGrid& grid, TTTPlayer turn)
{
   ...
   if(allEmpty)
   {
      ...
      moves.clear();
      unsigned int idx = unsigned int(0.5f * grid.size()); 
...
{
   assert(!moves.empty());
   unsigned int idx = rand() % (unsigned int)moves.size();
   return moves[idx];
}
...
getSetOfValues(const AIMOVES& moves, bool highest)
{
   ...
   unsigned int sz = (unsigned int)moves.size();
...
TTTAIUtils::getMinMove(const TTTGrid& grid, const TTTPlayerMove& previousMove, 
                       TTTPlayer turn, const NodeState& nodeToBeat, unsigned int depth, bool root)
{
   bool isRoot = root;
   root = false;
   ...
   else
   {
      ...
      unsigned int numMoves = (unsigned int)moves.size();
                                    ...
                                    unsigned int depth)
{
   ...
   else
   {
      ...
      numMoves = (unsigned int)taskObjects.size();
      for(unsigned int i=0; i<numMoves; ++i)      
      {
